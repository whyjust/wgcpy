...
def calculate_iv_woe(df, var, bin_size, abnormal_value_list, label):
    ...
    df_bin_info['woe'] = df_bin_info.apply(lambda x: np.log(x.good_per * 1.0 / (x.bad_per + 1e-5)), axis=1)
    df_bin_info['iv'] = df_bin_info.apply(lambda x:  (x.good_per - x.bad_per) * np.log(x.good_per * 1.0 / (x.bad_per + 1e-5)), axis=1)
